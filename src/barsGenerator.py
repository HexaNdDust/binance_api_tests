...
class Interface(object):
	def __init__(self, root, w_height = 480, w_width = 720, row = 0, column = 0, w_background = '#111821'):
	...
	def Normalize(self, p_max, p_min, delta_y = 0, delta_x = 0):
		...
		self.scale_y = (self.max_price - self.min_price) / (self.window_high - self.delta_y)
	...
	def draw_raw_grid(self, color):
		...
		for i in range(round(h_n)):
			self.canvas.create_line(0, int(h_size + i * h_size * 2), w_s, int(h_size + i * h_size * 2), fill = color)
		
		for i in range(round(w_n)):
			self.canvas.create_line(int(w_size + i * w_size * 2), 0, int(w_size + i * w_size * 2), h_s, fill = color)
	...
	def draw_bin_candle_chart(self, klines):
		for i in range(len(klines)):
			self.draw_candle(float(klines[i][2]), float(klines[i][3]), float(klines[i][1]), float(klines[i][4]), i)
